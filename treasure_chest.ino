/*
 * ============================================================================
 * ПРОЕКТ: "СУНДУК СОКРОВИЩ" - Тренинг биоуправления для коррекции внимания
 * ============================================================================
 * 
 * ЗАДАЧА:
 ...
 *   - Если бета-ритм > порога ≥60% времени → зажечь LEDs ("камни")
 *   - Периоды с высоким ЭМГ не засчитываются
 * 
 * КАЛИБРОВКА (30 сек):
 *   - Порог ЭМГ: среднее значение в покое × коэффициент
 *   - Порог ЭЭГ: максимум бета-ритма с открытыми глазами × коэффициент
 * 
 * ОБОРУДОВАНИЕ:
 *   - Arduino Mega
 ...
 * 
 * АВТОР: Генерация кода для задачи №6
 * ВЕРСИЯ: 2.0
 * ============================================================================
 */
...
const unsigned long SERIAL_INTERVAL     = 50;     // Отправка данных: 50 мс
...
// ПЕРЕМЕННЫЕ ЭЭГ
// ============================================================================
double eegReal[FFT_SIZE];    // Действительная часть
double eegImag[FFT_SIZE];    // Мнимая часть
double eegPower[FFT_SIZE];   // Мощность
uint8_t eegIdx = 0;
...
void sendToPC() {
  ...
  // Bitronics Lab читает значения построчно
  Serial.print(rawEEG);
  Serial.print('\t');
  Serial.print(rawEMG);
  Serial.print('\t');
...
void runTraining() {
  ...
  if (eegIdx == 0) {
    ...
    // (когда крышка открывается, а не закрывается)
  ...
  // Показываем текущий порог ЭМГ и бета-ритма
...
void loop() {
  ...
  // === ОТПРАВКА ДАННЫХ НА ПК (каждые 50 мс) ===
